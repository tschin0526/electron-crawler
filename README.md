...
ToDo 卡片默认保存在应用的 `data` 目录中。若需要将 `todo-*.json` 移到其他目录，可在项目根目录（`electron-crawler`）创建独立配置文件 `todolist-config.json`：

```json
{
   "todoDataDir": "/Users/你的用户名/Documents/todolist"
}
```

`todoDataDir` 支持绝对路径，也支持相对于默认 `data` 目录的路径，例如 `"todo-data"`。配置文件不存在、格式错误、目录参数为空或目标路径不是目录时，会自动回退到原来的默认 `data` 目录。配置在应用启动时读取，修改后请重启应用。

首次切换到新的 ToDo 目录时，默认 `data` 目录中已有的 `todo-*.json` 会自动迁移过去；目标目录中已有卡片时视为已初始化，不会覆盖或重新补回用户文件。
